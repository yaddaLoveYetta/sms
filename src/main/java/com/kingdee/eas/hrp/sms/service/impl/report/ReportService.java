...
import com.kingdee.eas.hrp.sms.dao.customize.ReportDaoMapper;
...
public class ReportService extends BaseService implements IReportService {
	...
	@Override
	public JSONObject getRecord(String itemId, String supplier, String orderStartDate, String orderEndDate, int pageNo, int pageSize) {
			...
			for (Map<String, Object> order : selectOrderStatistics) {
				...
				if (orderCount.containsKey(material)) {
					...
					if (!materialOrder.contains(materialOrderStr)) {
						...
						double qty = (double) orderCount.get(material) + Double.parseDouble(order.get("qty").toString());
					...
					if (!materialEntry.contains(materialEntryStr)) {
						double invoiceQty = (double) orderInvoiceQty.get(material) + Double.parseDouble(order.get("invoiceQty").toString());
					...
					if (!(null == order.get("returnQty") || "".equals(order.get("returnQty")))) {
						double returnQty = (double) purreturnsReturnQty.get(material) + Double.parseDouble(order.get("returnQty").toString());
						purreturnsReturnQty.replace(material, returnQty);
					}
					if (!(null == order.get("actualQty") || "".equals(order.get("actualQty")))) {
						double actualQty = (double) purinwarehsActualQty.get(material) + Double.parseDouble(order.get("actualQty").toString());
	...
	@Override
	public Map<String, Object> getItemLicense(Map<String, Object> params) {
		...
		ReportDaoMapper mapper = sqlSession.getMapper(ReportDaoMapper.class);
		...
		List<Map<String, Object>> list = mapper.getItemLicense(params);
		...
		// 还要查询出list中物料证件关联供应商的供应商资质 =======begin======================

		List<Map<String, Object>> supplierLicense = new ArrayList<Map<String, Object>>();

		// 拿到本次物料证件查询结果中的供应商
		List<String> supplies = new ArrayList<String>();

		for (Map<String, Object> item : list) {
			supplies.add("'" + item.get("supplierId") + "'");
		}

		if (!supplies.isEmpty()) {
			supplierLicense = mapper.getSupplierLicense(StringUtils.join(supplies, ","));
		}

		ret.put("supplierLicense", supplierLicense);

		// 还要查询出list中物料证件关联供应商的供应商资质 =======end========================
