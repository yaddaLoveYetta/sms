...
define('Edit', function (require, module, exports) {
    ...
    function save() {
        ...
        if (classId == 1001) {
            // 用户信息特殊校验逻辑
            // 供应商用户必须关联供应商
            if ((f7Selectors['type'].getData()[0].ID || '') == 'B3sMo22ZLkWApjO/oEeDOxACEAI='
                && (f7Selectors['supplier'].getData()[0].ID || '') == '') {
    ...
    FormEdit.on({
        'dialogChange': function (classId, key, data, selectors,
                                  metaData) {
            ...
            if (classId === 1001 && key === 'type') {
                ...
                if (data[0].ID == 'QpXq24FxxE6c3lvHMPyYCxACEAI=') {
                    ...
                    if ($(element).parents("td").siblings().find(
                            ".must-mark")) {
                        $(element).parents("td").siblings().find(
                            ".must-mark").remove();
                    }
                    ...
                    // 如果是必填需要添加 红色 * 号
                    if ($(element).parents("td").siblings().find(
                            ".must-mark").length <= 0) { // 如果不存在
                        var html = $(element).parents("td").siblings()
                            .html();
            ...
            if ((classId === 3010 || classId === 3020)
                ...
                for (var item in fields) {
                    ...
                    if (field.lookUpType === 3
                        && field.lookUpClassID === lookUpClassID) {
                        // 当前变化控件的属性携带
